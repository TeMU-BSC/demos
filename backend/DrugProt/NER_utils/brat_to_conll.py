...
def check_brat_annotation_and_text_compatibility(brat_folder):
    ...
    print(
        "Checking the validity of BRAT-formatted {0} set... ".format(dataset_type), end='')
    ...
    for text_filepath in text_filepaths:
        ...
        annotation_filepath = os.path.join(
            os.path.dirname(text_filepath), base_filename + '.ann')
        # check if annotation file exists
        if not os.path.exists(annotation_filepath):
            raise IOError("Annotation file does not exist: {0}".format(
                annotation_filepath))
        text, entities = get_entities_from_brat(
            text_filepath, annotation_filepath)
    print("Done.")
...
def brat_to_conll(input_folder, output_filepath, language):
    ...
    print("Formatting {0} set from BRAT to CONLL... ".format(
        dataset_type), end='')
