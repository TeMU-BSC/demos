...
import pandas as pd
from NER_utils import ann_parsing
import warnings
import os
...
def metrics_main(gs_path, pred_path, relevant_labels, subtask=['ner','norm'], filelist=[]):
    '''
    ...
    if len(filelist)>1:
        ann_list_gs = list(filter(lambda x: x[-3:]=='ann',filelist))
    else:
        ann_list_gs = list(filter(lambda x: x[-4:] == '.ann', os.listdir(gs_path)))
    subtask='ner' # FORCE THIS FOR OUR USE CASE 
    
    if subtask=='norm':
        gs = ann_parsing.main(gs_path, relevant_labels, with_notes=True)
        pred = ann_parsing.main(pred_path, relevant_labels, with_notes=True)
        
        if pred.shape[0] == 0:
            warnings.warn('There are not parsed predicted annotations')
        elif gs.shape[0] == 0:
            warnings.warn('There are not parsed Gold Standard annotations')
        ...
        gs = ann_parsing.main(gs_path, relevant_labels, with_notes=False)
        pred = ann_parsing.main(pred_path, relevant_labels, with_notes=False)
    ...
    # Remove predictions not in filelist
    if len(filelist)>1:
        gs = gs.loc[gs['clinical_case'].isin(filelist),:].copy()
        pred = pred.loc[pred['clinical_case'].isin(filelist),:].copy()
