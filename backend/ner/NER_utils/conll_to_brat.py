# -*- coding: utf-8 -*-
"""
from: https://github.com/TeMU-BSC/PharmaCoNER-Tagger
"""
import os
import codecs
import glob
...
def generate_reference_text_file_for_conll(conll_input_filepath, conll_output_filepath, text_folder):
    ...
    '''
    dataset_type =  get_basename_without_extension(conll_input_filepath)
    conll_file = codecs.open(conll_input_filepath, 'r', 'UTF-8')   
    create_folder_if_not_exists(text_folder)
    text = ''
    new_conll_string = ''
    character_index = 0
    document_count = 0
...
def check_compatibility_between_conll_and_brat_text(conll_filepath, brat_folder):
    ...
    '''
    verbose = False
    dataset_type = get_basename_without_extension(conll_filepath)
    ...
    for line in conll_file:
        ...
        token = {}
        token['text'] = str(' '.join(line[:-4]))
        token['start'] = int(line[-3])
        token['end'] = int(line[-2])
