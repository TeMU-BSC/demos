...
# version 2 using docker-py and dockerpty
# Author: Alejandro Asensio <alejandro.asensio.10@gmail.com>
# '''
...
#         # client.containers.run('bsctemu/freeling-cnio:1.0.0', detach=True, auto_remove=True, stdin_open=True)
        
#         client = docker.APIClient()
#         container = client.create_container(
#             image='bsctemu/freeling-cnio:1.0.0', stdin_open=True, tty=True)
#         self._tagger = subprocess.Popen('dockerpty.start(client, container)', shell=True,
#                                         stdin=subprocess.PIPE, stdout=subprocess.PIPE)

#         # client = docker.APIClient()
#         # self._tagger = client.create_container(
#         #     image='bsctemu/freeling-cnio:1.0.0',
#         #     stdin_open=True,
#         #     tty=True,
#         #     command='/bin/sh'
#         # )
#         # dockerpty.start(client, self._tagger)

#         # self._tagger = subprocess.Popen(dockerpty.start(client, self.container), shell=True,
#         #                                 stdin=subprocess.PIPE, stdout=subprocess.PIPE)
...
#     def get_freeling_results(self, text):
#         '''
...
#                     stop_it = stop_it + 1
#                     break
#                 freeling_results.append(
#                     tuple(output.decode('utf-8').split(' ')))
#             if freeling_results or stop_it > 3:
#                 break
#         return freeling_results
...
#         '''
#         freeling_results = self.get_freeling_results(text)
#         sentences = []
#         single_sentence = []
#         for item in freeling_results:
#             if len(item) < 3:
