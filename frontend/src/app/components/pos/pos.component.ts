...
export class PosComponent implements OnInit {
  ...
  columns = [
    new TableColumn<Word, 'sentenceId'>('Sentence #', 'sentenceId')
      .withColFilter()
      .withWidth(Width.pct(10))
      .isHiddenXs(true),
    new TableColumn<Word, 'id'>('Word #', 'id')
      .withColFilter()
      .withWidth(Width.pct(10))
      .isHiddenXs(true),
    new TableColumn<Word, 'forma'>(
      'Forma (original word)',
      'forma'
    ).withColFilter(),
    new TableColumn<Word, 'lemma'>('Lemma', 'lemma')
      .withColFilter()
      .withNgClass(() => 'font-italic'),
    new TableColumn<Word, 'tag'>('Tag', 'tag')
      .withColFilter()
      .withButton(ButtonType.RAISED)
      .withButtonColor('accent')
      // .withNgClass((data, parentData) => `tooltip ${parentData.pos}`)
      .withOnClick((tag, word) => {
    ...
    new TableColumn<Word, 'pos'>('POS category', 'pos')
      .withColFilter()
      .withButton(ButtonType.BASIC)
      .withButtonColor('primary')
      // .withNgClass((data, parentData) => `tooltip ${parentData.pos}`)
      .withOnClick((pos, word) => {
        const anchor = `${TAG_ANCHOR_BASE}${pos}`
        window.open(`${FREELING_USER_MANUAL_SPANISH_TAGSET_URL}${anchor}`)
      }),
    new TableColumn<Word, 'score'>('Score', 'score')
      .withColFilter()
      .withWidth(Width.pct(10)),
  ]
  ...
  ngOnInit() {
    ...
    // Init some atributes
    this.words = []
    this.downloadFilename = 'spaccc-pos-tagger.json'
  }
   ...
   */
  get f() {
    return this.freelingForm.controls
  }
